...
export const useExamAttempts = (userId, options = {}) => {
  ...
  const [cheatCounts, setCheatCounts] = useState({});
  ...
  const fetchAttempts = useCallback(async () => {
    ...
    try {
      ...
      if (withCheatCounts && data?.length) {
        ...
        setCheatCounts((cheatRows || []).reduce((acc, r) => {
      ...
      console.error('useExamAttempts error:', err);
      setError(err);
    } finally {
      setLoading(false);
    }
  }, [userId, status, excludeStatus, examType, dateFrom, dateTo, passFilter, page, pageSize, select, withCheatCounts, enabled]);

  useEffect(() => { fetchAttempts(); }, [fetchAttempts]);

  return { attempts, totalCount, cheatCounts, loading, error, refetch: fetchAttempts };
};
