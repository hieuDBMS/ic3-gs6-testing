...
const NavLink = ({ to, icon, children }) => {
  ...
  return (
    ...
    >
      <span className={`transition-colors ${active ? 'text-indigo-500' : 'text-gray-400'}`}>{icon}</span>
      {children}
      {active && (
...
const TeacherDropdown = () => {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Settings2 className={`w-4 h-4 transition-colors ${isAnyActive ? 'text-violet-500' : 'text-gray-400'}`} />
        Quản lý
      ...
      {open && (
        <div
          className="absolute top-full mt-2 left-0 w-64 bg-white rounded-2xl shadow-xl border border-gray-100/80 py-2 z-50"
          style={{ boxShadow: '0 8px 32px rgba(15,23,42,0.12), 0 2px 8px rgba(15,23,42,0.06)' }}
        >
          {/* Header */}
          <div className="px-4 pt-1 pb-2 mb-1 border-b border-gray-50">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Công cụ Giáo viên</p>
          </div>
          {items.map(item => {
              ...
              >
                <span className={`flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center transition-all
                  ${active
                    ...
                    : 'bg-gray-100 text-gray-400 group-hover:bg-violet-50 group-hover:text-violet-500'
                  }`}>
                  {item.icon}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold leading-none mb-0.5">{item.label}</p>
                  <p className="text-[11px] text-gray-400 leading-none truncate">{item.desc}</p>
                </div>
                <ChevronRight className={`w-3.5 h-3.5 flex-shrink-0 transition-all
                  ${active ? 'text-violet-400' : 'text-gray-300 group-hover:text-gray-400 group-hover:translate-x-0.5'}`}
                />
              </Link>
            );
          })}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-2">
              {/* User chip */}
              <div className="flex items-center gap-2.5 pl-2 pr-3.5 py-1.5 rounded-2xl bg-gray-50 border border-gray-200">
                ...
                <div className="leading-tight min-w-0">
                  <p className="text-[12px] font-semibold text-gray-800 truncate max-w-[100px]">{profile.full_name}</p>
              ...
              <button
                ...
                title="Đăng xuất"
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium text-gray-400 hover:text-red-500 hover:bg-red-50 border border-transparent hover:border-red-100 transition-all duration-150"
              >
                ...
                <>
                  ...
                  {teacherItems.map(item => {
                    ...
                    return (
                      ...
                      >
                        <span className={`${active ? 'text-violet-500' : 'text-gray-400'}`}>{item.icon}</span>
                        {item.label}
